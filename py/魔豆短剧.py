...
class Spider(SpiderBase):
        ...
        if str(tid) == self.SHORT_DRAMA_TID:
            ...
            if res.get("code") == 200:
                ...
                for item in data.get("items", []):
                    v_url = item.get("previewVideoUrl", "")
                    ep_cnt = item.get("episodeCount", 1)
                    rating = item.get("rating", 0.0)
                    videos.append({
                        "vod_id": f"drama@@{item.get('id')}@@{v_url}@@{item.get('title', '')}@@{item.get('coverUrl', '')}@@{ep_cnt}",
    ...
    def detailContent(self, ids: List[str]) -> Dict[str, Any]:
        """视频详情：支持普通视频单集与 AI 短剧全集播放列表"""
        vod_id = ids[0]

        if "@@" in vod_id:
            parts = vod_id.split("@@")
            vtype = parts[0]
            real_id = parts[1] if len(parts) > 1 else ""
            video_url = parts[2] if len(parts) > 2 else ""
            title = parts[3] if len(parts) > 3 else "视频详情"
            cover = parts[4] if len(parts) > 4 else ""

            if vtype == "drama":
                ep_cnt = int(parts[5]) if len(parts) > 5 and parts[5].isdigit() else 1
                play_stream = self._format_play_url(video_url)
                
                # 构建短剧选集列表
                episodes = [f"第1集${play_stream}"]
                for ep in range(2, ep_cnt + 1):
                    episodes.append(f"第{ep}集${play_stream}")
                
                play_url_str = "#".join(episodes)
                from_name = "AI短剧专线"
            else:
                play_stream = self._format_play_url(video_url)
                play_url_str = f"正片${play_stream}"
                from_name = "专线播放"
        else:
            title = "在线播放"
            cover = ""
