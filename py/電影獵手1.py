...
class Spider(Spider):
    ...
    def detailContent(self, ids):
        ...
        video = {'vod_name': data.get('name'),'type_name': data.get('type_name'),'vod_year': data.get('year'),'vod_area': data.get('area'),'vod_remarks': data.get('dynami'),'vod_content': "接口源码分享QQ交流群:212706934"+data.get('content')}
        play = []
        names = []
        tasks = []
